...
|-------|------|-------------|
| **DR** | Primary Support | Apply `corpflow:technical-lead` findings from `.context/developer-review-N.md`; enforce minimal-diff; write retries to `.context/errors/sys-code-fixer.md` |
...
### Consuming DR/QA gate-feedback on re-dispatch (corpflow v4.0.13)
...
You **consume** this contract — the injection itself is orchestrator-owned (corpflow `worktask/SKILL.md`). See `skills/_shared/workflow-integration/SKILL.md § Gate-Feedback Contract`.
